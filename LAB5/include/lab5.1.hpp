...
bool macierz::operator==(macierz mac)
{
    try
    {
        if(mac.rows()!=wiersz || mac.cols() !=kolumna)
    ...
    int licznik=0;
    int i,j;
    for(i=0;i<mac.rows();i++)
    {
        for(j=0;j<mac.cols();j++)
        {
            if(tab[i][j]==mac.tab[i][j]);
                licznik++;
        }
    }
    if(licznik==i*j)
...
double macierz::operator[](int number)
{
    try
    {
        if( number>wiersz)
